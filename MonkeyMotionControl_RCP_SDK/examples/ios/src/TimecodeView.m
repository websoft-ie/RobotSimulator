...
- (TimecodeView *)init
{
    if (self = [super init])
    {
        ...
        [_textField4 setEnabled:NO];
        [_textField4 setAlpha:0.5];
        [_textField4 setText:@"00"];
...
- (BOOL)isOpen
{
    return [_textField1 isFirstResponder] || [_textField2 isFirstResponder] || [_textField3 isFirstResponder];
}

- (int)timecode
{
    int hours = [[_textField1 text] intValue];
    int minutes = [[_textField2 text] intValue];
    int seconds = [[_textField3 text] intValue];
...
{
    int hours = timecode / 3600;
    int minutes = (timecode / 60) % 60;
    int seconds = timecode % 60;
    
    [_textField1 setText:[NSString stringWithFormat:@"%d", hours]];
    [_textField2 setText:[NSString stringWithFormat:@"%d", minutes]];
    [_textField3 setText:[NSString stringWithFormat:@"%d", seconds]];
}
...
- (void)sendRCPValue
{
    ...
    [handler rcpSetInt:paramID value:[self timecode]];
}
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    switch (textField.tag)
    {
        ...
        case 3:
            ...
            if (delegate != nil)
            {
                if ([delegate respondsToSelector:@selector(timecodeViewDoneEditing:)])
                {
                    [delegate timecodeViewDoneEditing:self];
                }
            }
            return YES;
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (value < 0 ||
       ([_textField1 isFirstResponder] && value > 23) ||
       ([_textField2 isFirstResponder] && value > 59) ||
       ([_textField3 isFirstResponder] && value > 59))
    {
        return NO;
    }
